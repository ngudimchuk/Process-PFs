...
if exist([folder,'/exludePFs.txt'])
    ...
    disp(['N = ',num2str(length(jexcl2)),' PFs',num2str(excl_pf),'were expluded from analysis by expludePFs.txt file']);
else
    jexcl2=[];
end
...
[L_init, ~]=PF_lengths_sub(Xt, Yt, Zt, maxk1, mode);
...
if excl_pf>0
    ...
    L=L_init;
    L(:,jexcl3)=[];
    disp(['N = ',num2str(length(jexcl3)),' PFs shorter than ',num2str(excl_pf),' dimers were expluded from analysis !']);
else
    L=L_init;
end
...
if term_curv>0
    disp(['mean terminal curvature: ', num2str(round(meancurvfromtip(1),1)),' +/- ', num2str(round(stdcurvfromtip(1),1)), ' deg/dimer']);
end
